...
export default async function SecurityGovernancePage() {
  ...
  return <div className="space-y-6">
    <PageHeader title="Security, Compliance & Governance" description="Control AI permissions, audit sensitive actions and manage privacy requests from one place." />
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <Card><div className="flex items-center justify-between"><h2 className="font-bold text-ink">Governance policy</h2><Badge tone="signal">Human controlled</Badge></div><form action={createGovernancePolicyAction} className="mt-4 space-y-3"><Input name="name" required placeholder="e.g. High-risk AI actions" /><Select name="policy_type" ><option>agent_actions</option><option>data_access</option><option>financial_actions</option><option>autonomous_operations</option></Select><label className="flex items-center gap-2 text-sm text-slate"><input type="checkbox" name="require_approval" defaultChecked /> Require human approval</label><Button type="submit">Create policy</Button></form></Card>
      <Card><h2 className="font-bold text-ink">Privacy & access requests</h2><p className="mt-1 text-sm text-slate">Requests are recorded for controlled processing; destructive deletion is never performed implicitly.</p><div className="mt-4 flex flex-wrap gap-2"><form action={requestDataAction}><input type="hidden" name="request_type" value="export"/><Button type="submit" variant="secondary" size="sm">Request data export</Button></form><form action={requestDataAction}><input type="hidden" name="request_type" value="access_review"/><Button type="submit" variant="secondary" size="sm">Request access review</Button></form><form action={requestDataAction}><input type="hidden" name="request_type" value="delete"/><Button type="submit" variant="danger" size="sm">Request account deletion</Button></form></div></Card>
    </div>
