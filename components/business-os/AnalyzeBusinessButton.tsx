...
export function AnalyzeBusinessButton() {
  ...
  return <div><Button type="button" onClick={run} disabled={state === "loading"}>{state === "loading" ? "Analyzing…" : "Analyze my business"}</Button>{message && <p className={`mt-2 text-xs ${state === "error" ? "text-danger" : "text-slate"}`}>{message}</p>}</div>;
}
