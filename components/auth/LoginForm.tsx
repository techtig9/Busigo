...
export function LoginForm({ justVerified }: { justVerified?: boolean }) {
  ...
  return (
    // The Google sign-in form is a SIBLING of the credentials form, not a child.
    // Nesting <form> inside <form> is invalid HTML: the parser drops the inner element, so
    // the hydrated client DOM no longer matched the server markup and React bailed out of
    // hydration on this page (errors #418 ×8 and #423 in a production build). Keeping the
    // two forms as siblings preserves both server actions and fixes the mismatch.
    <div className="space-y-4">
      ...
      {error && (
        <p role="alert" className="rounded border border-danger/30 bg-danger-soft px-3 py-2 text-sm text-danger-ink">
          {error}
        </p>
      )}
